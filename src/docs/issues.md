The issues that could not be solve have been added to this text block so the
...
was an issue with the *Keras* version and the removal of the engine object in
the new version of *Keras*. Also a this would have only worked for the first
tested model. So, a new $F_1\ Score$ metric was written instead.

**Torchtext Issue:**

There was problem with the compabitility of the torch version with torchtext.
There also was a problem with the python and cuda versions with a older version
of torch, but a reinstall was attempted.
